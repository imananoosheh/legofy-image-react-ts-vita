...
function App() {
    ...
    function propagateLegos() {
        ...
        const legoAndImageMarginWidth = imgWidth - (gridNumOfColumn*Number(sizeValue))
        const legoAndImageMarginHeight = imgHeight - (gridNumOfRow*Number(sizeValue))
        ...
        if (legoOverlayContainer) {
            const newStyle = `grid-template-columns:repeat(${gridNumOfColumn}, ${sizeValue}px);grid-template-rows:repeat(${gridNumOfRow}, ${sizeValue}px);inset: ${legoAndImageMarginHeight/2}px ${legoAndImageMarginWidth/2}px`;
    ...
    useEffect(() => {
        ...
        const legoBricks = document
            ...
            .forEach((brick) => {
                brick.setAttribute('style',`width:${sizeValue}px;height:${sizeValue}px`)
